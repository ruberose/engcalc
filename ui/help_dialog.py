...
<h3>표 블록</h3>
<p>메뉴 &gt; 파일 &gt; 표 삽입으로 기본 3행 3열 표를 넣습니다. 계산 기능은 없는
순수 텍스트 표입니다(단면 제원표, 하중 조합표 등에 사용). 칸을
<b>더블클릭</b>하면 그 칸만 편집되고, <b>Tab</b>을 누르면 바로 다음 칸
편집으로 이어집니다. 표를 <b>우클릭</b>하면 클릭했던 칸을 기준으로 행/열을
추가·삭제할 수 있고, 속성 패널의 "행 수"/"열 수"로 끝에서 추가·삭제할 수도
있습니다.</p>
...
<h3>목록(리스트)과 집계 함수</h3>
<p><code>[10, 20, 30]</code>처럼 대괄호로 여러 값을 묶어 변수에 담을 수 있습니다
(단위가 붙은 값도 가능: <code>[10 kN, 20 kN, 30 kN]</code>). 이 목록에
<code>sum, max, min, avg(mean과 같음), count</code> 함수를 쓸 수 있습니다.</p>
<pre>loads = [10 kN, 20 kN, 30 kN]
sum(loads)     -&gt;  60 kN
max(loads)     -&gt;  30 kN
avg(loads)     -&gt;  20 kN
count(loads)   -&gt;  3</pre>
<p>서로 다른 물리량(예: 힘과 길이)이 섞인 목록을 집계하려 하면 에러가 표시됩니다.</p>

<h3>사용자 정의 함수</h3>
<p><code>이름(매개변수, ...) = 본문</code> 형태로 자주 쓰는 계산식을 함수로
만들어두고 다른 블록에서 값만 바꿔가며 재사용할 수 있습니다.</p>
<pre>stress(F, A) = F / A
stress(200 kN, 300 mm * 500 mm)   -&gt;  1.33333 MPa</pre>
<p>함수 본문은 정의된 시점까지 위 블록에서 채워진 변수도 그대로 참조할 수
있습니다. 함수 정의 자체는(변수 대입처럼) 화면에 결과를 따로 보여주지
않고 입력한 그대로 표시됩니다. 매개변수 개수가 안 맞게 호출하거나, 본문에
정의되지 않은 변수를 쓰거나, 함수 이름 자체를 잘못 타이핑해서 호출하면
앱이 멈추지 않고 에러로 표시됩니다.</p>
